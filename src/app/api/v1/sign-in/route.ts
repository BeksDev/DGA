...
import { verifyPassword, generateToken } from '@app/util';
  ...
  try {
    ...
    if (!match) {
      ...
      roles.data.forEach((role: any) => {
        role.permissions.users = permissions?.data.users.filter((permission: any) =>
      ...
      roles.data.forEach((role: any) => {
        role.permissions.roles = permissions.data.roles.filter((permission: any) =>
